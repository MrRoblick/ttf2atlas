...
// FontToAtlas Starts at 32 characters
func FontToAtlas(FontPath string, FontSize float32) (*image.RGBA, error) {
	dpi := 72.0
	symbolSize := int(FontSize * float32(dpi) / 72)
	symbolSpacing := symbolSize / 4

	width := (symbolsPerWidth * symbolSize) + (symbolsPerWidth-1)*symbolSpacing
	height := (symbolsCount / symbolsPerWidth * symbolSize) + ((symbolsCount/symbolsPerWidth)-1)*symbolSpacing
	...
	for i := 0; i < symbolsCount; i++ {
		x = (i % symbolsPerWidth) * (symbolSize + symbolSpacing)
		y = (i / symbolsPerWidth) * (symbolSize + symbolSpacing)
		_, err = ctx.DrawString(string(rune(i)), freetype.Pt(x, y))
	}
	return img, nil
}
